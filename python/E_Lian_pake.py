...
from function import id_generate, pass_generate, key_padding, pass_change
...
def session():
    ...
    YA_new = YA * H3(pass_change.pass_change(pw_A))
    TA = time.time()
    # 将（XA、YA、TA）发送给B实例---B
    rB = rand()
    ...
    if not timestamp_is_fresh(TA):
        ...
        YA_1 = YA_new // H3(pass_change.pass_change(pw_B))
        tkB = XA ^ DB ^ id_B ^ H2(rB) ^ H2(H3(gmpy2.powmod(YA_1, rB, PRIME)))
            ...
            if dec_TB != TB:
                ...
                if YB != gmpy2.powmod(g, dec_rB, PRIME) or XB != H2(dec_hwB) ^ H2(dec_rB):
                    ...
                    if dec_TA != TA:
                        ...
                        if YA_1 != gmpy2.powmod(g, dec_rA, PRIME) or XA != H2(dec_hwA) ^ H2(dec_rA):
                            print("YA does not match or XA does not match")
                        else:
                            sidB = str(XA) + str(YA_1) + str(XB) + str(YB)  # XA、YA、XB、YB都是经过哈希或者幂运算的，都是int类型
